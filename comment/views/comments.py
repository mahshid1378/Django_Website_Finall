...
class CreateComment(BaseCommentView):
    ...
    def form_valid(self, form):
        ...
        if author_email:
            email = EmailMessage(
                        "New perspective",
                        "A new perspective for the article«{}» which you are the author of, has been sent:\n{}{}".format(article, current_site, reverse('blog:detail', kwargs={'slug': article.slug})),
                        to=[author_email]
            )
            email.send()
        ...
        if parent_email:
            email = EmailMessage(
                        ...
                        "A response to your comment in the article«{}» it is registered. Click on the link below to view:\n{}{}".format(article, current_site, reverse('blog:detail', kwargs={'slug': article.slug})),
                        to=[parent_email]
            )
            email.send()
